...
class Player extends SpriteAnimationGroupComponent with HasGameReference<PixelAdventure>, KeyboardHandler, CollisionCallbacks {
  ...
  late final SpriteAnimation idleAnimation;
    ...
    double hortizontalMovement = 0;
    double moveSpeed = 100;
    Vector2 velocity = Vector2.zero();
    bool isOnGround = false;
    bool hasJumped = false;
    List<CollisionBlock> collisionBlocks = [];
    CustomHitbox hitbox = CustomHitbox(width: 10, height: 2, offsetX: 14, offsetY: 28);
  ...
  bool onKeyEvent(KeyEvent event, Set<LogicalKeyboardKey> keysPressed) {
    hortizontalMovement = 0;
    final isLeftKeyPressed = keysPressed.contains(LogicalKeyboardKey.keyA) || keysPressed.contains(LogicalKeyboardKey.arrowLeft);
    final isRightKeyPressed = keysPressed.contains(LogicalKeyboardKey.keyD) || keysPressed.contains(LogicalKeyboardKey.arrowRight);

    hortizontalMovement += isLeftKeyPressed ? -1 : 0;
    hortizontalMovement += isRightKeyPressed ? 1 : 0;
  ...
  void _updatePlayerMovement(double dt) {
    ...
    velocity.x = hortizontalMovement * moveSpeed;
